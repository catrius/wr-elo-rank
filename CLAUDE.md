...
- **FeedbackBox** — Feature-request/feedback board. Reads/writes `feedback` + `feedback_vote`: insert feedback, toggle vote, edit own text, admin toggles status (open/done/joke). Status filter tabs, rc-pagination (PAGE_SIZE 10), markdown rendering (react-markdown + remark-gfm). Uses AuthContext, GameDataContext.
...
- **RankBadge** — Rank display: `—` when null; crown SVG (gold/silver/bronze via `MEDAL_COLORS`) with number overlaid for ranks 1–3; plain number otherwise. Internal `CrownIcon`.
