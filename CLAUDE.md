...
`PlayerGarden` (shown on `PlayerPage`) is an animated pixel-art garden scene visualizing a player's form. State is computed by `src/utils/garden.ts` and rendered against constants in `src/constants/garden.ts`; weather overlays, the lawn, and the flower beds live in `src/components/garden/`. Tree stage maps to season wins; weather maps to a composite health score (streak + recent form + win rate); poor records, long ice streaks, or Elo decay infest the tree with **pests** (bugs crawling on the canopy) rather than withering it.

The scene is a lawn receding to a horizon, not a flat ground strip: a 0–1 `depth` places each plant (and the tree, at `TREE_DEPTH`) along it, driving size, brightness, baseline y (`depthToY`), and whether it draws behind or in front of the tree. `Flora` takes a `layer` (`'back' | 'front'`) so paint order comes from mount position rather than z-index coordination, and the whole ground bed shares one `GROUND_LIGHT` filter so it can't be lit like noon under a night sky.

Asset sources: Aseprite masters for the pest sprites are in `assets/garden/pests/` (sources only, not served — see the README there for the grid layout and re-export steps). The static flora/dressing sprites are copied unmodified from the "Pixel Art Flower Pack" into `public/garden/flora/` (see the README there for the file→pack mapping; sprite dimensions are hard-coded as aspect ratios in `FLORA_SPRITES`/`GARDEN_PROPS`). `public/garden/lawn.png` is **generated** — rerun `assets/garden/lawn/generate.py` to change the lawn's gradient or grass speckle.
...
- Types: `GardenStage` (1–8), `WeatherState` (sunny/cloudy/rainy/stormy/blizzard), `PestKind` (locust/butterfly/beetle/wasp), `PestCause` (winrate/ice/decay), `FloraKind`, `Pest`, `Plant`, `WeatherBreakdown`, `GardenState`.
...
- `buildFlora(stage, weather, infested, playerId)` — Places the flower beds as `Plant[]` (`kind`, `x`, `depth` 0=horizon→1=nearest, `flipped`). Count is `FLORA_COUNT_BY_STAGE` × `FLORA_WEATHER_MULT` (bad weather thins the beds); species are drawn from the weighted `FLORA_POOL`; `x` uses even columns with in-column jitter (pure random clumped and left bald patches) and clears the trunk for plants near `TREE_DEPTH`. On an infested tree, `FLORA_DRY_SHARE` of bushes become `bushDry`. Deterministic per player.
- `computeGardenState(player, matches, playerId)` — Combines the above into `{ stage, weather, breakdown, causes, pests, plants }`.
...
- **FormatToolbar** — Markdown formatting toolbar with icon buttons for bold, italic, code, bullet list, numbered list, link, and image upload. Props: `onBold`, `onItalic`, `onCode`, `onBullet`, `onNumbered`, `onLink`, `onImage`, `uploading`. Used in FeedbackBox for textarea formatting.
...
- **FeedbackBox** — Feature-request/feedback board. Reads/writes `feedback` + `feedback_vote`: insert feedback, toggle vote, edit own text, admin toggles status (open/done/joke). Status filter tabs, rc-pagination (PAGE_SIZE 10), markdown rendering (react-markdown + remark-gfm) with image support. Image upload via `FormatToolbar` inserts markdown syntax directly into text. Author names respect the ingame toggle via `displayName`. Uses AuthContext, DisplayNameContext, GameDataContext.
- **PlayerGarden** — Animated pixel-art garden visualizing a player's form (tree stage + weather + pests + flower beds). Computes state via `computeGardenState` (memoized); `ResizeObserver` scales a fixed 480×320 design canvas as one unit; `requestAnimationFrame` tree sway (summed sine waves) applied to a wrapper holding both the tree image and the `Pests` overlay, so bugs ride the canopy. Tree box size comes from `STAGE_HEIGHTS` × `STAGE_ASPECT` (no image measuring); the trunk's base sits at `depthToY(TREE_DEPTH)`. Scene layer order: sky → weather overlay → (`Lawn` → `Flora back` → soil bed + shadow → tree → `Flora front`), the parenthesized group wrapped in the `GROUND_LIGHT` filter. Weather → overlay: sunny→`SunRays` + `Birds`, cloudy/stormy→`Clouds` (dark when stormy), rainy→`RainDrops`, stormy→`RainDrops heavy` + `Lightning`, blizzard→`Blizzard`. Info panel + admin-only debug panel (`DebugSelect` for stage/weather, `DebugCauses` for pest causes; the readout names the species actually rendered and flags any the stage's bug cap dropped). Any of the three debug overrides rebuilds the flora, since it keys off stage, weather, and infestation. Props: `player`, `matches`, `playerId`, `isAdmin?`.
...
- **RankBadge** — Rank display: `—` when null; crown SVG (gold/silver/bronze via `MEDAL_COLORS`) with number overlaid for ranks 1–3; a horned winged-heart emblem (`DevilEmblem`) with the rank number drawn over the heart when `bottomRank` (1–3, dead-last first) is set — its two fills are recolored per rank via `DEVIL_COLORS` (distinct hues: infernal red for dead-last, cursed purple, toxic green); shared horns/shading stay fixed; plain number otherwise. Internal `CrownIcon`, `DevilEmblem`.
...
- **Lawn** — Ground plane + backdrop dressing, replacing the old tiled 48px dirt strip. Draws the baked `lawn.png` slab (1:1 at `DESIGN_W`), the horizon trellises from `GARDEN_PROPS`, then a mirrored-alternating hedge row tiled from the bush sprite to hide the slab's top edge. Everything it renders sits at or behind the horizon, so it mounts once beneath the beds; no props in the near corners (cropped pots read as pasted on). No props.
- **Flora** — Flower beds. Renders the `Plant[]` whose `depth` falls on the requested side of `TREE_DEPTH`, sizing/dimming each by depth so the lawn recedes, and z-indexing by depth so nearer plants overlap farther ones. All plants share one `flora-sway` keyframe track varied per plant by a `--flora-angle` custom property and a negative delay; it animates the standalone `rotate` property (not a transform) so it composes with the mirrored plants' `scaleX` instead of overwriting it. Sway timing derives from `x`/`depth` rather than a stored seed. Props: `plants`, `layer`.
